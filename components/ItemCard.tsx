...
export const ItemCard: React.FC<ItemCardProps> = ({ item, onEdit }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-zinc-800 relative overflow-hidden transition-all duration-200 active:scale-[0.99]">
      ...
      <div className="flex justify-between items-start mb-4 pl-2">
        ...
        <div className="flex flex-col items-end">
           <span className={`text-2xl font-mono font-bold tracking-tight ${isLowStock ? 'text-accent' : 'text-primary dark:text-zinc-100'}`}>
             {item.quantity}
      ...
      <div className="flex items-center justify-between gap-4 mt-2 pl-2">
        <div className="flex items-center gap-2">
             {isLowStock && (
               <div className="flex items-center gap-1 text-xs font-medium text-accent bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-full">
                 <AlertCircle size={12} />
                 补货
               </div>
             )}
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="w-12 h-10 flex items-center justify-center rounded-xl bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 active:bg-gray-200 dark:active:bg-zinc-700 active:text-gray-900 transition-colors"
